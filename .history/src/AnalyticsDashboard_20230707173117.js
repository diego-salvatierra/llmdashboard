...
const AnalyticsDashboard = () => {
  ...
  async function fetchApiData() {
    let { data: fetchedData, error, count } = await supabase
      ...
      .order('created_at', { ascending: false }) // Order by the created_at column
      .range(0, 999); // Fetch the first 1,000 rows
    ...
    while (fetchedData.length < count) {
      const { data: additionalData, error } = await supabase
        ...
        .select("type, model, cost, created_at")
        .order('created_at', { ascending: false }) // Order by the created_at column
        .range(fetchedData.length, fetchedData.length + 999); // Fetch the next 1,000 rows
