...
function generateUsersData() {
  ...
  data.forEach((entry) => {
    ...
    if (!usersData[user]) {
      ...
      usersData[user]++;
      if (usersData[user] === 2) { // This was a first-time user but now they're returning
        firstTimeUsers--;
        returningUsers++;
      }
    }
  ...
  return { firstTimeUsers, returningUsers };
}

const usersData = generateUsersData();

const returningUserChartData = [
  {name: 'First-Time Users', count: usersData.firstTimeUsers},
  {name: 'Returning Users', count: usersData.returningUsers},
];
...
<h2>Returning User Activity</h2>
<BarChart
  width={500}
  height={300}
  data={returningUserChartData}
  margin={{
    top: 20,
    right: 30,
    left: 20,
